...
export default function AnalysisPanel({ data, loading }: AnalysisProp) {
  const [activeTab, setActiveTab] = useState<TabType>("explanation");
  const [copied, setCopied] = useState(false);

  if (!loading && (!data || data.length === 0)) {
    return (
      <p className="text-slate-400 text-center font-bold text-2xl">
        No analysis yet.
      </p>
    );
  }
  ...
  const getActive = () => {
    ...
    if (activeTab === "explanation") return items.explanation || "";

    if (activeTab === "fixed") return items.fixed_Code || "";

    if (activeTab === "steps") return items.how_To_Fix || "";
        ...
        <div className="rounded-lg border border-blue-900/30 bg-slate-900/50 backdrop-blur-sm overflow-hidden shadow-lg">
          {loading ? (
            <div className="p-6 flex flex-col items-center justify-center min-h-96">
              <FaReact className="text-blue-400 w-14 h-14 animate-spin" />
              {/* <div className="w-8 h-8 border-3 border-blue-500/30 border-t-blue-500 rounded-full animate-spin mb-4"></div> */}
              <p className="text-slate-400 text-lg">Analyzing your code...</p>
            </div>
          ) : (
            <>
              {activeTab === "explanation" && data[0] && (
                <AnalysisTab
                  title="Explanation"
                  content={data[0]?.explanation}
                  type="text"
                />
              )}
              {activeTab === "fixed" && data[0] && (
                <AnalysisTab
                  title="Fixed Code"
                  content={data[0]?.fixed_Code}
                  type="code"
                  onCopy={copyActive}
                />
              )}
              {activeTab === "steps" && data[0] && (
                <AnalysisTab
                  title="How to Fix"
                  content={
                    data[0]?.how_To_Fix
                      ? data[0].how_To_Fix.split(".").filter(Boolean)
                      : [
                          <span className="flex items-center gap-2  text-md">
                            <GoAlertFill size={25} className="text-amber-300" />
                            No steps provided for this error.
                          </span>
                        ]
                  }
                  type="list"
